...
namespace QSynth
{
    ...
    FSeries* FSeries::RandomFSeries(int complexity, Waves* wave)
    {
        ...
        double amplitude = exp(-pow(10*(pitch-1), 2));
        return new FSeries(pitch, HarmMapping::RandomHarmMapping(complexity, wave), wave, n, amplitude, chi(rng));
    };
    ...
    void SynthNote::ToWAV(const char* file_name)
    {
        short* buff = Buffer();
        int nSamples = static_cast<int>((dur+ADSR.R) * sample_rate);
    ...
    XSynth::XSynth(SynthNote* synth_note, std::vector<int> notes, std::vector<int> vels, std::vector<int> rels) :
        midinotes(notes)
    {
        sn = synth_note;
        harm_sn = nullptr;
    ...
    XSynth::XSynth(SynthNote* synth_note, SynthNote* harm_note, std::vector<int> notes, std::vector<int> vels, std::vector<int> rels) :
        midinotes(notes)
    {
        sn = synth_note;
        harm_sn = harm_note;
    ...
    XSynth::XSynth(SynthNote* synth_note, SynthNote* harm_note, SynthNote* rel_note, std::vector<int> notes, std::vector<int> vels, std::vector<int> rels) :
        midinotes(notes)
    {
        sn = synth_note;
        harm_sn = harm_note;
    ...
    void XSynth::CreateMap(std::vector<int> vels, std::vector<int> rels)
    {
        this->clear();
        dynamics d;
        articulation art;
    ...
    void XSynth::SaveSamplesToBitKlavier(std::string sample_name, bool override)
    {
